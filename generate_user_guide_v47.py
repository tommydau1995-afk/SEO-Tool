"""Generate the detailed Vietnamese guide bundled with SEO AI Studio V4.7."""
...
def generate(path=OUTPUT):
    ...
    if normal_path and bold_path:
        pdfmetrics.registerFont(TTFont("V47Guide", normal_path))
        pdfmetrics.registerFont(TTFont("V47GuideBold", bold_path))
        normal_font, bold_font = "V47Guide", "V47GuideBold"
    ...
    doc = BaseDocTemplate(
        ...
        topMargin=18 * mm,
        bottomMargin=15 * mm,
        title="Hướng dẫn chi tiết SEO AI Studio V4.7",
    ...
    def header_footer(canvas, document):
        ...
        canvas.drawString(
            15 * mm,
            height - 7.2 * mm,
            "SEO AI STUDIO V4.7 - HƯỚNG DẪN CHI TIẾT",
        )
    ...
    body = styles["V47Body"]
    small = styles["V47Small"]
    story = []

    def h1(text):
        return Paragraph(safe(text), styles["V47H1"])

    def h2(text):
        story.append(Paragraph(safe(text), styles["V47H2"]))
    ...
    def callout(title, text, color=PALE_BLUE, border=BLUE):
        table = Table(
            [[Paragraph(f"<b>{safe(title)}</b><br/>{text}", styles["V47Callout"])]],
            colWidths=[doc.width],
        )
        table.setStyle(
            TableStyle(
    ...
    story.append(Paragraph("SEO AI STUDIO V4.7", styles["V47Title"]))
    story.append(
        Paragraph(
    ...
    metrics = [
        ...
        ("3 nguồn", "Keyword Discovery"),
        ("30/60/90", "SEO Roadmap"),
    ]
    metric_table = Table(
        [
            [Paragraph(value, styles["V47Metric"]) for value, _ in metrics],
            [Paragraph(label, styles["V47MetricLabel"]) for _, label in metrics],
        ],
    ...
    callout(
        ...
        "Giúp bạn cài đặt, thiết lập, tự tìm bộ keyword không cần Semrush, chạy Audit, tạo Content Brief, "
        "xuất WordPress, theo dõi kết quả và xử lý lỗi mà không cần biết lập trình.",
        PALE_GREEN,
        GREEN,
    )
    story.append(p("<b>Phiên bản:</b> 4.7.0 &nbsp;&nbsp; <b>Nền tảng:</b> Windows 10/11 64-bit"))

    # 2
    page("1. Cài đặt và mở ứng dụng", "Bộ cài đã chứa ứng dụng và file hướng dẫn này.")
    for number, title, text in (
        (1, "Chạy bộ cài", "Mở SEO_AI_Studio_V47_Setup.exe và chọn Next."),
        (2, "Chọn Desktop icon", "Bật tùy chọn nếu muốn có biểu tượng ngoài Desktop."),
        (3, "Hoàn tất", "Chọn Launch SEO AI Studio V4.7."),
